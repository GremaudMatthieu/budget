...
export default function EnvelopeDetailScreen() {
  ...
  useEffect(() => {
    ...
    const handleSocketEvent = (event) => {
      ...
      if (event.aggregateId === uuid || event.budgetEnvelopeId === uuid) {
        ...
        // Calculate time since last API fetch
        const timeSinceLastFetch = now - lastApiFetchTimestampRef.current;
        
        // If we recently did an API fetch or have a pending user action, delay this refresh
        let delayTime = 0;
        if (timeSinceLastFetch < 300) {
          delayTime = 500; // Add extra delay if we just fetched data
        } else if (pendingActionTimestamp && now - pendingActionTimestamp < 1000) {
          delayTime = 1200; // Longer delay if user just performed an action
        }
        
        // Use a delay to ensure we don't refresh too early
        setTimeout(() => {
          if (isMounted.current) {
            loadEnvelopeDetails();
          }
  ...
  }, [socket, connected, uuid, loadEnvelopeDetails, router, setError, pendingActionTimestamp]);
